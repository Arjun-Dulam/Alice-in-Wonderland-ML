...
class FullyConnectedLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        \begin{itemize}
            \item \tbf{Forward Automatic Differentiation} is a concept wherein we build up the gradient for the loss function at the very las layer starting from the first layer.
            \item Consider an example:
            \[\mbf{h} = f_1(\mbf{x}), \mbf{y} = f_2(){\mbf{h}})\]
            Their gradients are:
            \[\partial_\mbf{x}\mbf{y} = \partial_\mbf{h}\mbf{y} \cdot \partial _\mbf{x} \mbf{h}\]
            We start by computing the first layer and its gradient, so $f_1$ and $\partial_\mbf{x} \mbf{h}$ will be computed. We then compute the gradient for $f_2$ and multiply it with the gradient of the previous layer.
            Continuing this cycle for subsequent layers will result in the gradient for the total loss function. 
        \end{itemize}
\end{document}